...
func createSession(ctx context.Context, request *imageBuildRequest, tracer *buildKitBuildTracer, cfg *configfile.ConfigFile) (*session.Session, error) {
	...
	authProvider, err := createAuthProvider(tracer, cfg)
...
func createAuthProvider(tracer *buildKitBuildTracer, cfg *configfile.ConfigFile) (session.Attachable, error) {
	authProvider := authprovider.NewDockerAuthProvider(cfg)
...
func resetUIDAndGID(path string, stat *fsutiltypes.Stat) fsutil.MapResult {
	stat.Uid = 0
	stat.Gid = 0
